...
Public Class magazzinoMasterPage
    ...
    Private Sub magazzinoMasterPage_Init(sender As Object, e As EventArgs) Handles Me.Init
        If Not CType(Session("user"), user) Is Nothing AndAlso (CType(Session("user"), user).iSmagazzino Or CType(Session("user"), user).iSmagazzinoadmin) Then    'utente amministratore
        Else
            Try
                Session.Abandon()
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '    ASPxMenu_Top.Items.FindByName("linkordineinpicking").Enabled = True
        'Else
        '    ASPxMenu_Top.Items.FindByName("linkordineinpicking").Enabled = False
        'End If
        ASPxMenu_Top.Items.FindByName("raccogliordine").Enabled = (CType(Session("user"), user).iSmagazzino Or CType(Session("user"), user).iSmagazzinoadmin)
        ASPxMenu_Top.Items.FindByName("linkordineinpicking").Enabled = ((CType(Session("user"), user).iSmagazzino Or CType(Session("user"), user).iSmagazzinoadmin) And CType(Session("carrelloMagazzino"), cartManager).Header.ordineHeader.Code <> "")
        ASPxMenu_Top.Items.FindByName("ordinidaraccogliere").Enabled = CType(Session("user"), user).iSmagazzinoadmin
        ASPxMenu_Top.Items.FindByName("ordiniinlavorazione").Enabled = CType(Session("user"), user).iSmagazzinoadmin Or CType(Session("user"), user).iSmagazzino
        ASPxMenu_Top.Items.FindByName("controlloefatturazione").Enabled = CType(Session("user"), user).iSmagazzinoadmin
	ASPxMenu_Top.Items.FindByName("assegnaordini").Enabled = CType(Session("user"), user).iSmagazzinoadmin
