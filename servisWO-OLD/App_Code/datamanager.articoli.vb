...
Partial Public Class datamanager
    ...
    Public Function getQtaInLavorazione(Optional ByVal CodiceArticolo As String = "") As DataTable
        ...
        Try
            ...
            conn.Open()
            Dim sql As String
            sql = "SELECT B.[ItemCode], SUM(B.[Quantity]+B.[DiscountQty]) AS qtaLavorazione FROM [carrelloHeader] AS A with(NOLOCK) "
            sql &= " INNER Join [carrelloLines] AS B with(NOLOCK) ON A.Code=B.OrderCode And A.[CompanyName_]=B.[CompanyName_] "
            sql &= " WHERE A.[Status] <> 6 AND A.[CompletedImported]=0 AND A.TIPO = 'ORDINE' AND (A.PRENOTAZIONE=0 OR A.PRENOTAZIONE=2) "
    ...
    Public Function getQtaInLavorazionePerLotto(Optional ByVal CodiceArticolo As String = "") As DataTable
        ...
        Try
            ...
            conn.Open()
            Dim sql As String
            sql = "SELECT B.[ItemCode],B.[LotNo],SUM(B.[Quantity]+B.[DiscountQty]) AS qtaLavorazione FROM [carrelloHeader] AS A with(NOLOCK) "
            sql &= " INNER JOIN [carrelloLines] As B With(NOLOCK) On A.Code=B.OrderCode And A.[CompanyName_]=B.[CompanyName_] "
            sql &= " WHERE A.[Status] <>6 AND A.[CompletedImported]=0 AND A.TIPO = 'ORDINE' AND (A.PRENOTAZIONE=0 OR A.PRENOTAZIONE=2) "
            sql &= " AND A.[CompanyName_]=@CompanyCode "
            If CodiceArticolo <> "" Then
