...
Partial Public Class datamanager
    ...
    Public Function shipOrder(ByVal orderCode As String, ByVal UserCtrl As String, ByVal peso As Integer, ByVal nrcolli As Integer, ByVal ShippingAgentCode As String) As Integer
        Dim conn As New SqlConnection(WORconnectionString) ' Using only WORconnectionString since they're the same.
        conn.Open()
        Try
            Dim cmd As New SqlCommand
            cmd.Connection = conn
            ...
            sql = "UPDATE [OrderHeaderFromWeb_Write] SET [OrderNoCtrl]=@OrderNoCtrl, [UserCtrl]=@UserCtrl,[ShippingAgentCode]=@ShippingAgentCode,[PackageNum]=@PackageNum,[Weight]=@Weight,[Status]=@Status WHERE [Code]=@Code AND [CompanyName_]=@CompanyName"
            cmd.CommandText = sql
            cmd.Parameters.Clear()
